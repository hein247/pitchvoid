...
const OUTPUT_SECTIONS = [
  { title: "The Opportunity", preview: "The AI infrastructure market is projected to reach $407B by 2028..." },
  { title: "Traction & Metrics", preview: "$2M ARR with 40% month-over-month growth across 127 enterprise..." },
  { title: "The Ask", preview: "We're raising $15M to expand our engineering team and accelerate..." },
];
...
const appleSlow = { duration: 0.9, ease: appleEase };
const appleMedium = { duration: 0.65, ease: appleEase };
const appleFast = { duration: 0.45, ease: appleEase };

const dofVariants = {
  enter: { opacity: 0, scale: 1.06, filter: "blur(12px)", y: 0 },
  center: {
    opacity: 1, scale: 1, filter: "blur(0px)", y: 0,
    transition: {
      opacity: { duration: 0.7, ease: appleEase },
      scale: { duration: 0.9, ease: appleEase },
      filter: { duration: 0.8, ease: appleEase },
    },
  },
  exit: {
    opacity: 0, scale: 0.94, filter: "blur(12px)", y: 0,
    transition: {
      opacity: { duration: 0.5, ease: [0.25, 0.1, 0.7, 1] as [number, number, number, number] },
      scale: { duration: 0.6, ease: [0.25, 0.1, 0.7, 1] as [number, number, number, number] },
      filter: { duration: 0.5, ease: [0.25, 0.1, 0.7, 1] as [number, number, number, number] },
    },
  },
};
...
const cardMorphItem = {
  hidden: { opacity: 0, scale: 0.82, y: 30, filter: "blur(8px)", rotateY: -8 },
  visible: {
    opacity: 1, scale: 1, y: 0, filter: "blur(0px)", rotateY: 0,
    transition: {
      opacity: { duration: 0.6, ease: appleEase },
      scale: { duration: 0.8, ease: appleEase },
      y: { duration: 0.8, ease: appleEase },
      filter: { duration: 0.7, ease: appleEase },
      rotateY: { duration: 0.9, ease: appleEase },
    },
  },
};
...
const outputItem = {
  ...
  visible: {
    ...
    transition: {
      opacity: { duration: 0.6, ease: appleEase },
      x: { duration: 0.75, ease: appleEase },
      filter: { duration: 0.65, ease: appleEase },
    },
  },
};
...
function ParallaxLayer({
  ...
  return (
    <motion.div style={{ x: springX, y: springY, ...style }} className={className}>
      {children}
    </motion.div>
  );
}
...
export default function HowItWorks() {
  ...
  const glowPositions = useMemo(() => [
    { x: "35%", y: "40%", color: "rgba(168,85,247,0.07)" },
    { x: "55%", y: "35%", color: "rgba(139,92,246,0.08)" },
    { x: "60%", y: "50%", color: "rgba(168,85,247,0.06)" },
  ], []);
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{ width: 800, height: 800, borderRadius: "50%", transform: "translate(-50%, -50%)" }}
      />
      ...
      {/* Header */}
      <ParallaxLayer depth={0.5} mouseX={mouseX} mouseY={mouseY} className="text-center relative z-[1]" style={{ marginBottom: 72 }}>
        <motion.div
      ...
      <div className="flex gap-14 max-w-[1400px] w-full relative z-[1] flex-wrap">
          ...
          {STEPS.map((step, idx) => {
            ...
            return (
              ...
              >
                {/* progress bar */}
                <motion.div
                  className="absolute bottom-0 left-4 right-4 h-[2px] rounded-[1px]"
                  style={{
                  ...
                  transition={{ scaleX: { duration: 0.05 }, opacity: appleMedium }}
                />
        ...
        <ParallaxLayer depth={0.8} mouseX={mouseX} mouseY={mouseY} className="flex-1 min-w-[420px] min-h-[550px] relative max-lg:min-w-0 max-lg:min-h-[470px]">
          <motion.div
            animate={{
              boxShadow: [
                "0 0 60px rgba(168,85,247,0.04), 0 0 120px rgba(168,85,247,0.02)",
                "0 0 80px rgba(168,85,247,0.06), 0 0 160px rgba(168,85,247,0.03)",
                "0 0 60px rgba(168,85,247,0.04), 0 0 120px rgba(168,85,247,0.02)",
              ],
            }}
          ...
          >
            ...
            <motion.div
              ...
              transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
              className="absolute top-0 left-[10%] right-[10%] h-px z-[3]"
              style={{ background: "linear-gradient(90deg, transparent, rgba(168,85,247,0.4), transparent)" }}
            />

            {/* vignette */}
            <div
              className="absolute inset-0 pointer-events-none z-[2] rounded-3xl"
              style={{ background: "radial-gradient(ellipse at center, transparent 50%, rgba(8,6,16,0.4) 100%)" }}
            />
              ...
              {activeStep === 2 && (
                ...
                >
                  ...
                  <motion.div
                    ...
                    transition={{ duration: 0.85, delay: 0.2, ease: appleEase }}
                    className="rounded-2xl overflow-hidden"
                    style={{ border: "1px solid rgba(168,85,247,0.12)", background: "rgba(8,6,16,0.5)" }}
                  >
                    <div
                    ...
                    <motion.div variants={outputStagger} initial="hidden" animate="visible">
                      {OUTPUT_SECTIONS.map((section, idx) => (
                        <motion.div
                          key={section.title}
                          variants={outputItem}
                          style={{
