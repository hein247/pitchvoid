...
import { Pencil, Type, List, Phone, ChevronLeft, X, Send, Loader2 } from 'lucide-react';
...
const MobileEditorSheet = ({ data, onUpdate, onRefine, isRefining = false }: MobileEditorSheetProps) => {
  ...
  const handleRefineSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (refineValue.trim() && !isRefining) {
      onRefine(refineValue.trim());
      setRefineValue('');
      handleClose();
    }
  };
                  ...
                  <div className="space-y-3">
                    <form onSubmit={handleRefineSubmit} className="relative">
                      <input
                        ...
                        disabled={isRefining}
                        className="w-full bg-accent/5 border border-border rounded-xl px-4 py-3 pr-12 text-foreground placeholder-muted-foreground text-sm focus:outline-none focus:border-primary/50 transition-colors disabled:opacity-50"
                      />
                      <button
                        type="submit"
                        disabled={!refineValue.trim() || isRefining}
                        className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-lg text-primary hover:bg-primary/10 transition-colors disabled:opacity-50"
                      >
                        {isRefining ? (
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                      {QUICK_CHIPS.map((chip) => (
                        <button
                          ...
                          disabled={isRefining}
                          className="flex-shrink-0 px-3 py-1.5 bg-accent/5 hover:bg-accent/10 border border-border rounded-full text-xs text-muted-foreground hover:text-foreground transition-colors disabled:opacity-50"
                        >
                          {chip}
                        </button>
                      ))}
